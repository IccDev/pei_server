...
impl Serve<AnnuaireSearch> for DatabaseService {
    ...
    async fn handle(&self, message: AnnuaireSearch, _system: &ServiceAssistant<Self>) -> Self::Response {
        match message {
            AnnuaireSearch::ByKey(key) => self.user_langues_by_key(key).await,
            AnnuaireSearch::ById(id) => self.user_langues_by_id(id).await
        }
    }
}

impl DatabaseService {
    pub(crate) async fn user_langues_by_key(&self, msg: String) -> Result<ResponseData, String> {
    
        Err(String::with_capacity(0))
    }

    pub(crate) async fn user_langues_by_id(&self, msg: i32) -> Result<ResponseData, String> {
