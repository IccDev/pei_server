...
pub(crate) async fn query_branches_user_id_route(req: Request<IncomingBody>, params: Params) -> Response<BoxedBody> {
    match params.get("user_id") {
        Some(user_id) => DB.query_branches_by_user(user_id.to_owned()).await,
