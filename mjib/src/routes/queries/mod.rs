...
pub fn get_user_sections_by_user_id(id: i32) -> impl Reply {
    let mut db = DB.access();
    let res = DB.get_user_sections_by_user_id(id, &mut db);
    match res.is_ok() {
        true => {
            Response::new(format!("{:#?}", serde_json::json!(res.unwrap()).to_string()).into())
        }
...
pub fn get_academic_years() -> impl Reply {
    let mut db = DB.access();
    let res = DB.get_academic_years(&mut db);
    match res.is_ok() {
        true => {
            Response::new(format!("{:#?}", serde_json::json!(res.unwrap()).to_string()).into())
        }
